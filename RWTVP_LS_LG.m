...
function draws = RWTVP_LS_LG(y, x, burnin, ndraws, ind_SV, ind_sparse, ind_forecast)
% Inputs:
...
for drawi = 1:ntotal   
    ...
    ind2_old = ind2;
    logrw_old = logrw;     
    [pz, ind2, logrw, count_pz, pz_mean, pz_cov] = pz_simulator_LS_LG(y, x,...
        vary, r0, a, v2, pz_old, ind2_old, b0_mean, b0_cov, pstar, AMH_c, logrw_old,...
...
%     % Asis-z: compute zhat
%     zhat = repmat(a',n,1) .* pz;
%     diff_zhat = zhat(2:n,:) - zhat(1:(n-1),:);
%     diff_zhat2 = diff_zhat.^2;
%     zhat12 = (zhat(1,:)'.^2)./(1+r0);
%     
%     
%     % Asis-z: update z02 (SA)
%     a2 = zeros(K,1);
%     a_sign = sign(a);
%     for j = 1:K 
%         [a2(j),~] = gigrnd(0.5-0.5*n, 1/psik(j), zhat12(j)+sum(diff_zhat2(:,j)), 1);
%         if a2(j) == 0
%             a2(j) = 1e-100;
%         end          
%     end
