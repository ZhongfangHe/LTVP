...
%
% square link: d_jt = max(0, z_jt^2-cj)
% for identification, sj = 1



function draws = RWTVP_LS_SQ(y, x, burnin, ndraws, ind_SV, ind_sparse, ind_forecast)
% Inputs:
...
%% Priors: threshold, c ~ N(0, diag(psik))
...
pz = zeros(n,K);
tmp = pz.^2 - repmat(c',n,1);
ind2 = repmat(v2',n,1).*double(tmp>0).*tmp;
...
draws.c = zeros(ndraws,K);
draws.count_c = 0;
draws.logrw_c = zeros(ndraws,1); %c
...
for drawi = 1:ntotal   
    ...
    ind2_old = ind2;
    logrw_old = logrw;     
    [pz, ind2, logrw, count_pz, pz_mean, pz_cov] = pz_simulator_LS_SQ(y, x,...
        vary, r0, c, v2, pz_old, ind2_old, b0_mean, b0_cov, pstar, AMH_c, logrw_old,...
        logrw_start, drawi_start, drawi, burnin, pz_mean_old, pz_cov_old); 

    
    % Draw c (SA, integrating out beta， multivariate)
    count_c = 0;
    ...
    if drawi < 100
        ...
        A = c_cov + 1e-6 * eye(K) / drawi; %add a small constant
    end
    eps = mvnrnd(zeros(K,1),A)'; %correlated normal
    c_new = c_old + exp(logrw_c) * eps; 
    
    c2_old = c_old.^2;
    c2_new = c_new.^2;
    logprior_old = -0.5 * sum(c2_old./psik);%p(z0)
    logprior_new = -0.5 * sum(c2_new./psik);

    w_old = ind2;
    loglike_old = loglike_TVP2(y, x, vary, w_old, beta0);
    tmp = pz.^2 - repmat(c_new',n,1);
    w_new = repmat(v2',n,1) .* double(tmp>0) .* tmp;
    ...
    p = exp(min(0,logprob));
    ei = max(200, drawi/K);
    ei_start = max(200, drawi_start_c/K);
    d = max(ei - ei_start, 20);
    logrwj = logrw_c + AMH_c * (p - pstar)/d;
    if abs(logrwj - logrw_start_c) > 1.0986 %log(3) ~= 1.0986 
        drawi_start_c = drawi;
        logrw_start_c = logrw_c;
    end %restart when useful to allow for larger movement    
    logrw_c = logrwj; %update proposal stdev
    
    
    c_mean_old = c_mean;
    c_cov_old = c_cov;
    c_mean = (c_mean_old * (drawi-1) + c) / drawi;
    c_cov = (drawi - 1) * (c_cov_old + c_mean_old * c_mean_old') / drawi + ...
        c * c' / drawi - c_mean * c_mean'; %update the sample covariance
...
%     psi = tau * phi;
    ...
    if drawi > burnin
        ...
        draws.count_c = draws.count_c + count_c; 
        draws.logrw_c(i) = logrw_c;%c         
...
draws.count_c = draws.count_c / ndraws;
