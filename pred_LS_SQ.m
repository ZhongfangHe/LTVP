...
% Use after running "RWTVP_LS_SQ"
% Simulate sig2_tp1 and z_tp1

function [ytp1_pdf, ytp1_mean, ytp1_var, ytp1_pdf_vec, ind_valid] = pred_LS_SQ(draws, xtp1,...
    ytp1, ind_SV)
% Inputs:
...
for drawi = 1:ndraws
    ...
    ci = draws.c(drawi,:)';
    tmp = ztp1.^2 - ci;
    ind_tp1 = double(tmp>0).*tmp; %compute ind_tp1
    vi = draws.v(drawi,:)';
    vi2 = vi.^2;
